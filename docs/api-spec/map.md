...
| 필드 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| category | string | N | `CAFE` / `RESTAURANT` / `PARK` (코드 기준 enum) |
...
| category | string | Y | `CAFE` / `RESTAURANT` / `PARK` (코드 기준 enum — 향후 확장 시 `app/models/enums.py:StoreCategory` 갱신) |
