...
### 5.12 좌표 → 행정구역 변환 — `GET /geo/reverse` [T1]

**인증 필요**

브라우저 Geolocation 또는 모바일 앱 GPS로 받은 좌표를 카카오 Local API로 reverse-geocode 한다. 홈 화면의 위치 pill, 매장 등록 시 자동 동 추출 등에 사용.
...
- `is_in_siheung`: `sido == "경기도" AND si == "시흥시"` 일 때 `true`.
- `label`: 시흥시 내부면 동 이름만, 외부면 `"{dong} (시흥시 외부)"`. 클라이언트가 그대로 표시하면 정책 통일.
...
| 401 | 인증 실패 |
| 404 | 카카오 응답에 `documents`가 비어 있음 (해상·해외 등) |
