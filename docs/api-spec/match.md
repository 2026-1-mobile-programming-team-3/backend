...
공통 사항(Base URL, 헤더, 에러 코드 등)은 `auth.md` 참고. 라우터 코드: `app/api/v1/matches.py` (prefix `/matches`, tag `Matches`).

> 채팅 REST 엔드포인트(`POST/GET .../messages`, `GET .../chats`)는 매칭 라우터에 내포되어 §3.10~3.12 로 정리한다.
> WebSocket 채널은 `chat.md` (별도 `chats` 라우터, prefix `/ws`).
> `/users/me/matches` 와 `/users/me/volunteer-stats` 는 본 문서에서 분리되어 `users.md` 의 §3.1·§2.2 로 이동했다.
...
### 3.14 / 3.15 — `users.md` 로 이동

`/users/me/volunteer-stats` 와 `/users/me/matches` 는 `users` 라우터 소속이므로 `users.md` 의 §2.2 와 §3.1 로 분리되었다. 라우터 코드: `app/api/v1/users.py`.
