...
| content | string | Y | 요청 글 본문 (1자 이상) |
| latitude | float | Y | 위도 (-90 ~ 90) |
| longitude | float | Y | 경도 (-180 ~ 180) |
...
| content | string | N | 글 내용 (1자 이상) |
| latitude | float | N | 위도. `longitude`와 함께 보내야 함 |
| longitude | float | N | 경도. `latitude`와 함께 보내야 함 |
| address | string | N | 주소 (최대 255자) |
| desired_date | string | N | 희망 날짜 (`YYYY-MM-DD`) |
| pet_id | integer | N | 반려동물 id (본인 소유) |
